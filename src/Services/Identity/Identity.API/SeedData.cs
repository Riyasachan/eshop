...
public class SeedData
{
    public static async Task EnsureSeedData(IServiceScope scope, IConfiguration configuration, ILogger logger)
    {
        ...
        await retryPolicy.ExecuteAsync(async () =>
        {
            ...
            var bobby = await userMgr.FindByNameAsync("bobby");

            if (bobby == null)
            {
                bobby = new ApplicationUser
                {
                    UserName = "bobby",
                    Email = "BobbySmith@email.com",
                    EmailConfirmed = true,
                    CardHolderName = "Bobby Smith",
                    CardNumber = "4012888888881889",
                    CardType = 1,
                    City = "Redmond",
                    Country = "U.S.",
                    Expiration = "12/24",
                ...
                var result = await userMgr.CreateAsync(bobby, "Pass123$$");
                ...
                logger.LogDebug("bobby created");
            }
            else
            {
                logger.LogDebug("bobby already exists");
            }
